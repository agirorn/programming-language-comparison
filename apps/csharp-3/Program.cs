...
app.MapGet("/hello", () =>
{
    return Results.Ok("Hello from c# 3");
});
...
app.MapGet("/count", async (NpgsqlConnection connection) =>
{
    ...
    var count = await connection.QueryAsync<int>("SELECT count(*) from csharp_3");
...
app.MapPost("/insert", async (NpgsqlConnection connection, RawData body) =>
{
    ...
    string sql = $"INSERT INTO csharp_3 (id, data) VALUES (@id, @data)";
